<?php

/* 
26. Вам нужно создать массив и заполнить его случайными числами от 1 до 100 (ф­я rand). 
Далее, вычислить произведение тех элементов, которые больше ноля и у которых парные индексы. 
После вывести на экран элементы, которые больше ноля и у которых не парный индекс.
*/

$arr = [];
$res = 1;
for($i=1; $i<=100; $i++){
	$arr[$i] = rand(1, 100);
}

var_dump ($arr);

foreach ($arr as $key => $elem){
	if(!($key % 2) && !($key==0) && $elem >0){
		$res *= $elem;
	}
}
echo "<br>";
echo "Произведение элементов, которые больше ноля и у которых парные индексы - " . $res;

foreach ($arr as $key => $elem){
	if(($key % 2) && $elem >0){
		echo "<br>";
		echo "Элемент, которые больше ноля и у которых не парный индекс - " . $elem;
	}
}
